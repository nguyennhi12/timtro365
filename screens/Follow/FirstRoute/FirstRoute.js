...
import { styles } from './FirstRoute.styles';
import {ListItem } from 'react-native-elements';
import { Avatar} from 'react-native-paper';
import IconBack from 'react-native-vector-icons/AntDesign'
import{getfollower} from '../../../hook/Account'
export default function FirstRoute(data) {
    const{follow,setcheckfollow,checkfollow}=getfollower()
    console.log(follow)
    return (
        <View >
            {
                follow!=null?<View>
                    <Text>Đang theo dõi: {follow.length}</Text>
                    <ScrollView>
                        {follow.map(item=>(
